    ...
    App.prototype.update = function()
    {
        ...
        */
        ...
        while(++i < bn)
        {
            ...
            while(++j < pn)
            {
                buffer.setDiameter(j,GLKit.Math.rect(Math.sin(j/pn*Math.PI+ time * 2))*(0.0125 + Math.abs(Math.sin(i/bn*Math.PI*4+time)*0.025)));
            }
            ...
            {
                k = -1;
                while(++k < buffer.getNumSegments())
                {

                    l = (j * buffer.getNumSegments() + k) * 3;
                    x = buffer.vertices[l];
            ...
            gl.drawElements(buffer.vertices,null,gl.fillColorBuffer(gl.getColorBuffer(),buffer.colors),null,buffer.indices,gl.LINES);
